...
class BucketModelOptimizer:
    ...
    Attributes:
        ...
        _model_copy (BucketModel): A copy of the original model instance.
        ...
        folds (int): The number of folds for the n-folds method.
    """
    ...
    def set_options(self, method: str, bounds: dict, folds: int = 1) -> None:
        """
        Sets the optimization method and bounds for the calibration.
    ...
    def single_fold_calibration(
        ...
        Returns:
            list[float]: A list of the calibrated parameters.
        """
    ...
    def calibrate(
        ...
        """
        Calibrates the model's parameters using the specified method and bounds.

        Args:
            initial_guess (list[float]): A list of initial guesses for the parameters. If not provided, uniform random values are sampled from the bounds.
            verbose (bool): A boolean indicating whether to print the current parameter values at each iteration.
    ...
    def score_model(self, metrics: list[str] = ["nse"]) -> dict:
        """
        Calculates the goodness of fit metrics for a given model.

        Args:
            metrics (list[str], optional): A list of strings containing the names of the metrics to calculate. If no metrics are provided, only NSE is calculated.
    ...
    def plot_of_surface(
        ...
        """
        Creates a 2D plot of the objective function surface for two parameters.
